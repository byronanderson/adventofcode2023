...
const parse = (input: string) => {
  return input
    ...
    .split("\n")
    .map((line) => {
      const [_, idString, game] = line.match(
        /^Game (\d+): (.*)$/,
      ) as RegExpMatchArray;
      const pulls = game.split("; ").map((pullString) => {
        const obj = Object.fromEntries(
          pullString.split(",").map((datum) => {
            const [color, number] = datum.split(" ").reverse();
...
const step1 = (
  input: string,
  bagConfiguration: { red: number; green: number; blue: number },
) => {
  return parse(input)
    .filter((game) =>
      game.pulls.every(
        (pull) =>
          pull.red <= bagConfiguration.red &&
          pull.blue <= bagConfiguration.blue &&
          pull.green <= bagConfiguration.green,
      )
    )
    .reduce((x, y) => x + y.id, 0);
};
...
`;
Deno.test("step1", () => {
  assertEquals(step1(testInput, { red: 12, green: 13, blue: 14 }), 8);
  console.log(
    "step 1 result",
    step1(input(), { red: 12, green: 13, blue: 14 }),
  );
});

const step2 = (input: string) => {
  return parse(input)
    .map((game) => {
      const red = Math.max(...game.pulls.map((pull) => pull.red));
      const green = Math.max(...game.pulls.map((pull) => pull.green));
      const blue = Math.max(...game.pulls.map((pull) => pull.blue));
...
Deno.test("step2", () => {
  assertEquals(step2(testInput), 2286);
  console.log("step 2 result", step2(input()));
});
