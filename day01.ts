...
const step1 = (input: string) => {
  return input
    ...
    .split("\n")
    .map((line) => {
      const digits = line.trim().replaceAll(/[^\d]/g, "");
...
Deno.test("step1", () => {
  ...
  assertEquals(step1("nrhdxfsqvxcbcghf35eightthreeseven5"), 35);
  console.log("step 1 result", step1(input()));
});

const replace = (line: string): string => {
  let result: RegExpMatchArray | null;
  if ((result = line.match(/^(\d*)$/))) {
    return line;
  } else if ((result = line.match(/^(\d*)o(ne.*)$/))) {
    return replace(result[1].concat("1").concat(result[2]));
  } else if ((result = line.match(/^(\d*)t(wo.*)$/))) {
    return replace(result[1].concat("2").concat(result[2]));
  } else if ((result = line.match(/^(\d*)t(hree.*)$/))) {
    return replace(result[1].concat("3").concat(result[2]));
  } else if ((result = line.match(/^(\d*)f(our.*)$/))) {
    return replace(result[1].concat("4").concat(result[2]));
  } else if ((result = line.match(/^(\d*)f(ive.*)$/))) {
    return replace(result[1].concat("5").concat(result[2]));
  } else if ((result = line.match(/^(\d*)s(ix.*)$/))) {
    return replace(result[1].concat("6").concat(result[2]));
  } else if ((result = line.match(/^(\d*)s(even.*)$/))) {
    return replace(result[1].concat("7").concat(result[2]));
  } else if ((result = line.match(/^(\d*)e(ight.*)$/))) {
    return replace(result[1].concat("8").concat(result[2]));
  } else if ((result = line.match(/^(\d*)n(ine.*)$/))) {
    return replace(result[1].concat("9").concat(result[2]));
  } else if ((result = line.match(/^(\d*)\w(.*)$/))) {
    return replace(result[1].concat(result[2]));
  } else {
    throw new Error("what case is this?");
  }
};
...
const step2 = (input: string) => {
  return input
    ...
    .split("\n")
    .map((line) => {
      const digits = replace(line.trim());
      const first = digits[0];
      const last = digits[digits.length - 1];
      return parseInt([first, last].join(""));
    })
